...
const PRICE_POLL_MS = 30000;   // fetch prices every 30 seconds
...
export class SignalService {
  ...
  private startPricePoll() {
    this.fetchPrices();
    this.priceInterval = setInterval(() => this.fetchPrices(), PRICE_POLL_MS);
  }
  ...
  private fetchPrices() {
    fetch(API_URL)
      ...
      .then(data => {
        if (data?.prices) {
          this.ngZone.run(() => this.prices$.next(data.prices));
        }
      })
